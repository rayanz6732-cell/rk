// Vercel serverless function — replaces base44 animekaiStream
// Deploy this to Vercel: /api/stream
...
export default async function handler(req, res) {
  ...
  if (!mal_id || !episode) return res.status(400).json({ error: 'mal_id and episode required' });
  ...
  if (!slugs.length) return res.status(404).json({ error: 'Anime not found', title: searchTitle });

  const slug = pickSlug(slugs, isDub);
  const src = await getEpisodeSrc(slug, episode);
  if (!src) {
    for (const fb of slugs.slice(1, 4)) {
      const fbSrc = await getEpisodeSrc(fb, episode);
      if (fbSrc) return res.json({ src: fbSrc, slug: fb, title: searchTitle });
    }
    return res.status(404).json({ error: 'Episode not found', slug, title: searchTitle });
  }

  return res.json({ src, slug, title: searchTitle });
}